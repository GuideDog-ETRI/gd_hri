...
def _get_mapping(erp_W, erp_H, theta, hfov, vfov):
    """
    캐시에 매핑이 있으면 그대로 반환,
    없으면 새로 만들어 캐시에 저장 후 반환.
    """
    # 실수(부동소수) 비교 오차를 막기 위해 적당히 반올림
    key = (erp_W, erp_H, round(float(theta), 8), round(float(hfov), 8), round(float(vfov), 8))
    
    if key not in _mapping_cache:
        _mapping_cache[key] = create_erp_to_rect_mapping(
            erp_W, erp_H, theta, hfov, vfov
        )
...
def erp_to_rect(erp_image, theta, hfov, vfov, erp_xi=None, erp_yi=None):
    erp_H, erp_W = erp_image.shape[:2]
    if erp_xi is None or erp_yi is None:
        erp_xi, erp_yi = _get_mapping(erp_W, erp_H, theta, hfov, vfov)
