...
class MySupportsWebcam(Protocol):
...
class MyWebcam:
    ...
    def _send_http_no_validate(self, endpoint: MyWebcam.Endpoint, **kwargs) -> requests.Response:
    ...
    def _send_http(self, endpoint: MyWebcam.Endpoint, **kwargs) -> requests.Response:
    ...
    def start(
        ...
        logging.info("MyWebcam started successfully")
...
class MyPlayer(Node):
    """The MyWebcam Stream Viewer via Open CV

    Note that each player runs in a separate process.
    """

    def __init__(self, topic_name = None, qos_profile = None) -> None:
        super().__init__('gopro_talker')   # Node name

        """Constructor"""
        self._vid: MySupportsWebcam
    ...
    def _run(self) -> None:
        ...
        # ros
        self._ros_publisher = self.create_publisher(Image, 
                                                    self.topic_name, 
                                                    self.qos_profile)
        self._ros_bridge = CvBridge()
        self.get_logger().info(f'create_publisher - topic_name: {self.topic_name})')
        self.get_logger().info(f'create_publisher - qos_profile: {self.qos_profile}')
        ...
        self.get_logger().info(f'videocapture url: {self.url}?overrun_nonfatal=1&fifo_size=50000000')
        ...
        logging.info("MyPlayer started.")

        while not self._stop_player.is_set():
            ret, frame = self._vid.read()

            if ret:
                print('publisher_gopro: ', frame.shape)

                # ROS
                if self._ros_bridge is not None and self._ros_publisher is not None:
                    msg = self._ros_bridge.cv2_to_imgmsg(frame, 'bgr8')
                    self._ros_publisher.publish(msg)
                    self.get_logger().info('publisher_gopro.publish GoPro Frame')
                    # cv.imshow("publisher", frame)
                else:
                    self.get_logger().warn('Ros_bridge or ros_publisher has none')
            else:
...
class MyGoProWebcamPlayer:
    ...
    def __init__(self, serial: str, port: Optional[int] = None,
                 topic_name: Any = None, qos_profile: Any = None) -> None:
        """Constructor
        ...
        """
        self.serial = serial
        self.webcam = MyWebcam(serial)
        self.player = MyPlayer(topic_name, qos_profile)
        if port and port in MyGoProWebcamPlayer._used_ports:
            raise RuntimeError(f"Port {port} is already being used")
        self.port = port or MyGoProWebcamPlayer._get_free_port()
        MyGoProWebcamPlayer._used_ports.add(self.port)
        logging.debug(f"Using port {self.port}")

    def __enter__(self) -> MyGoProWebcamPlayer:
        self.open()
        return self
    ...
    def play(self, resolution: Optional[int] = None, fov: Optional[int] = None) -> None:
        """Configure and start the GoPro MyWebcam. Then open and display the stream.
        ...
        self.player.start(MyGoProWebcamPlayer.STREAM_URL.format(port=self.port))
